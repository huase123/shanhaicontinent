 ...
 * @author Ben
 */
public class GenLayerTFBiomesSIX extends GenLayer {

	private static final int RARE_BIOME_CHANCE = 2;

	protected static final List<Supplier<Biome>> commonBiomes = Arrays.asList(
			() -> HanderBiome.biomeSHPlanins_SIX
	);
	protected static final List<Supplier<Biome>> rareBiomes = Arrays.asList(
			() -> HanderBiome.biomeSHPlanins_SIX,
			() -> HanderBiome.biomeSHOcean_SIX,
			() -> HanderBiome.biomeSHNether_SIX,
			() -> HanderBiome.biomeSHIce_SIX,
			() -> HanderBiome.biomeSHHills_SIX
	);

	public GenLayerTFBiomesSIX(long l, GenLayer genlayer) {
		super(l);
		parent = genlayer;
	}

	public GenLayerTFBiomesSIX(long l) {
		super(l);
	}

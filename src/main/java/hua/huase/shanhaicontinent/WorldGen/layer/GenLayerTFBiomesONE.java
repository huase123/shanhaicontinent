 ...
 * @author Ben
 */
public class GenLayerTFBiomesONE extends GenLayer {

	private static final int RARE_BIOME_CHANCE = 2;

	protected static final List<Supplier<Biome>> commonBiomes = Arrays.asList(
			() -> HanderBiome.biomeSHPlanins_ONE
	);
	protected static final List<Supplier<Biome>> rareBiomes = Arrays.asList(
			() -> HanderBiome.biomeSHPlanins_ONE,
			() -> HanderBiome.biomeSHOcean_ONE,
			() -> HanderBiome.biomeSHNether_ONE,
			() -> HanderBiome.biomeSHIce_ONE,
			() -> HanderBiome.biomeSHHills_ONE
	);

	public GenLayerTFBiomesONE(long l, GenLayer genlayer) {
		super(l);
		parent = genlayer;
	}

	public GenLayerTFBiomesONE(long l) {
		super(l);
	}

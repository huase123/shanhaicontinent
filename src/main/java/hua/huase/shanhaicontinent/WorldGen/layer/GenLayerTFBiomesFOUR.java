 ...
 * @author Ben
 */
public class GenLayerTFBiomesFOUR extends GenLayer {

	private static final int RARE_BIOME_CHANCE = 2;

	protected static final List<Supplier<Biome>> commonBiomes = Arrays.asList(
			() -> HanderBiome.biomeSHPlanins_FOUR
	);
	protected static final List<Supplier<Biome>> rareBiomes = Arrays.asList(
			() -> HanderBiome.biomeSHPlanins_FOUR,
			() -> HanderBiome.biomeSHOcean_FOUR,
			() -> HanderBiome.biomeSHNether_FOUR,
			() -> HanderBiome.biomeSHIce_FOUR,
			() -> HanderBiome.biomeSHHills_FOUR
	);

	public GenLayerTFBiomesFOUR(long l, GenLayer genlayer) {
		super(l);
		parent = genlayer;
	}

	public GenLayerTFBiomesFOUR(long l) {
		super(l);
	}

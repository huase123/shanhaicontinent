 ...
 * @author Ben
 */
public class GenLayerTFBiomesFIVE extends GenLayer {

	private static final int RARE_BIOME_CHANCE = 2;

	protected static final List<Supplier<Biome>> commonBiomes = Arrays.asList(
			() -> HanderBiome.biomeSHPlanins_FIVE
	);
	protected static final List<Supplier<Biome>> rareBiomes = Arrays.asList(
			() -> HanderBiome.biomeSHPlanins_FIVE,
			() -> HanderBiome.biomeSHOcean_FIVE,
			() -> HanderBiome.biomeSHNether_FIVE,
			() -> HanderBiome.biomeSHIce_FIVE,
			() -> HanderBiome.biomeSHHills_FIVE
	);

	public GenLayerTFBiomesFIVE(long l, GenLayer genlayer) {
		super(l);
		parent = genlayer;
	}

	public GenLayerTFBiomesFIVE(long l) {
		super(l);
	}

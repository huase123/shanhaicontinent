...
public interface HuitEventApi {
    ...
    static float otherHuitPlayer(PlayerCapability capability,float amount){
    ...
    static float otherHuitMon(MonsterCapability capability, float amount){
        float v1 = Math.max(capability.getShanbi(), 0f) / (Math.max(capability.getShanbi(), 0f) + 100f);
        if((float)random.nextInt(100)/100f<=v1){
            return 0f;
        }
    ...
    static float playerHuitOther(PlayerCapability capability,float amount,EntityLivingBase entityLiving){
    ...
    static float monHuitOther(MonsterCapability capability, float amount,EntityLivingBase entityLiving){
        boolean b = random.nextInt(100) <= capability.getBaojilv();
        float v = b? (amount+capability.getWugong()+capability.getZhenshang()) * Math.max((capability.getBaojishanghai()+100f)/100f,1f):amount+capability.getWugong()+capability.getZhenshang();
        if(!entityLiving.isDead)
        entityLiving.setHealth(capability.getXixue()/(capability.getXixue()+100)*v+entityLiving.getHealth());
    ...
    static float monHuitPlayer(MonsterCapability capability0, PlayerCapability capability1, float amount, EntityLivingBase entityLiving){
    ...
    static float monHuitmon(MonsterCapability capability0,MonsterCapability capability1, float amount, EntityLivingBase entityLiving){
        float v1 = Math.max(capability1.getShanbi() - capability0.getMinghzong(), 0f) / (Math.max(capability1.getShanbi() - capability0.getMinghzong(), 0f) + 100f);
        if((float)random.nextInt(100)/100f<=v1){
            return 0f;
        }
    ...
    static float playerHuitMon(PlayerCapability capability0,MonsterCapability capability1, float amount, EntityLivingBase entityLiving){
        float v1 = Math.max(capability1.getShanbi() - capability0.getMinghzong(), 0f) / (Math.max(capability1.getShanbi() - capability0.getMinghzong(), 0f) + 100f);
        if((float)random.nextInt(100)/100f<=v1){
            return 0f;
        }
